# CLAUDE.md — SYGNIF execution-layer repo (Windows)

Project memory for Claude Code working in `C:\Users\giank\sygnif\` —
the Freqtrade containers + Trade Overseer + Finance Agent codebase.

The full system specification (architecture, doctrine, ops) lives in
the agent repo at `~/sygnif/sygnif-agent/SYGNIF.md` (X1) and is
synced into the body below. **Do not edit between the SYNC markers —
edits will be overwritten.** To change the spec, edit `SYGNIF.md` on
X1 then run `bin/sync-docs.sh --push-windows`.

This wrapper file is regenerated by:
```bash
ssh x1 'cd ~/sygnif/sygnif-agent && bin/sync-docs.sh --push-windows'
```

The execution-layer-only sections (Freqtrade strategies, docker compose,
trade-overseer commentary path) appear AFTER the SYNC block.
...
## Execution-layer specifics (this repo only)

The system spec above covers the full SYGNIF agent + brain + executor.
What follows is specific to **this repo's Freqtrade trading containers
on EC2**, which run alongside the SYGNIF agent stack.

### Components in this repo

| Component | Where | Purpose |
|---|---|---|
| `SygnifStrategy.py` | repo root + `user_data/strategies/` | Spot strategy — NFI-derived, Claude sentiment |
| `MarketStrategy.py` | `user_data/strategies/` | Futures strategy (`freqtrade-futures` since `e388c2d`) |
| `trade_overseer/` | repo root | Telegram trade monitor + commentary (commentary slot is the SYGNIF brain language plan §6.3 target) |
| `finance_agent/` | repo root | Briefing + strategy-router HTTP service (host port 8091) |
| `notification_handler.py` | repo root | Webhook fan-out |
| `docker-compose.yml` | repo root | 4 containers: `freqtrade`, `freqtrade-futures`, `notification-handler`, `trade-overseer` |
| `user_data/config.json` | repo root | Spot config (port 8080, `dry_run: true`) |
| `user_data/config_futures.json` | repo root | Futures config (port 8081, isolated margin, 2-5x lev, `dry_run: true`) |

These Freqtrade strategies are **distinct from** the SYGNIF agent's
`agent/loop.py` planner. They share the same EC2 host but write to
separate Bybit accounts and separate trade journals.

### SygnifStrategy quick reference

#### Entry tags

| Tag | Side | Trigger |
|---|---|---|
| `strong_ta` | Long | TA score >= 65 + volume > 1.2× SMA25 |
| `strong_ta_short` | Short | TA score <= 25 |
| `claude_s{N}` | Long | TA 40-70 + Claude sentiment, combined >= 55 |
| `claude_short_s{N}` | Short | TA 30-60 + Claude sentiment, combined <= 40 |
| `claude_swing` / `claude_swing_short` | both | Failure swing + TA confirmation |
| `swing_failure` / `swing_failure_short` | both | Failure swing standalone |

#### Exit logic

- Profit-tiered RSI exits (leverage-aware)
- Williams %R overbought/oversold at > 2% profit
- Ratcheting trailing stop: -3% at +2%, -2% at +5%, -1.5% at +10%, BE guard at +1%
- Soft stoploss at -12% P&L (RSI-slope-confirmed; configurable via `soft_sl_ratio_*`)
- Doom stoploss: -20% P&L (price-divided by leverage on futures)

#### Risk management

- Doom cooldown: 4h per pair after SL hit
- Consecutive-loss lockout: 2+ SL on same pair within 24h → 24h block
- Slot caps: max 6 strong_ta, max 4 swing trades concurrent
- Futures volume gate: vol_sma_25 > 50k (except swings)
- Multi-TF RSI cascade blocks entries during crashes (long) / pumps (short)

#### Leverage tiers (futures)

- Majors (BTC, ETH, SOL, XRP): 5×
- Default: 3×
- ATR > 3%: capped at 2×; ATR > 2%: capped at 3×

### Deploy (Freqtrade stack only)

```bash
# Push SSH key (Instance Connect, 60s window) — see also docs/AWS_SSM_SESSION_MANAGER.md
aws ec2-instance-connect send-ssh-public-key \
...
# Pull + restart trading containers
ssh ec2-eu1 "cd ~/sygnif && git pull && \
  docker compose restart freqtrade freqtrade-futures trade-overseer"
```

**Note**: `trade_overseer/systemd/trade-overseer.service` runs on the host
out of `/home/ubuntu/trade_overseer`, separate from the docker compose stack.

### Important repo rules

- Strategy files exist in two places (repo root + `user_data/strategies/`) and **must stay in sync**.
- Freqtrade caches the loaded strategy at container start — **always restart** after edits.
- Both configs are `dry_run: true`. Set to `false` only after explicit user confirmation.

### Trade Overseer commentary path

`trade_overseer/` LLM backend is pluggable:

- `OVERSEER_AGENT_URL` — preferred. **Once SYGNIF language plan §6.3 ships**, point this at the brain commentary endpoint (`http://ec2-eu1:8889/api/commentary`). Until then, `http://host.docker.internal:8091/overseer/commentary` (the legacy host-side finance agent).
- `ANTHROPIC_API_KEY` — legacy fallback.
- Rules-only summary if no backend reachable.

Telegram token priority: `SYGNIF_HEDGE_BOT_TOKEN` > `FINANCE_BOT_TOKEN` > `TELEGRAM_BOT_TOKEN`.

`trade_overseer/touch_rate_tracker.py` reports per-entry-family hit-rate of the +1% breakeven-arming threshold:

```bash
ssh ec2-eu1 "cd ~/sygnif && python3 trade_overseer/touch_rate_tracker.py"
# variants: --days 7  --side long  --threshold 0.02  --no-print
```

### Inspect Freqtrade state

```bash
# Recent futures exits
...
What to watch:
- `exit_reason` containing `stoploss` should be a small fraction
- `close_profit` on SL exits should cluster near -0.12 (soft) or -0.20 (doom), not -0.03 to -0.05
- Trail exits should show frequent +1% to +3% captures
...
| File | Purpose |
|---|---|
| `SygnifStrategy.py` | Spot strategy source (root copy) |
| `user_data/strategies/SygnifStrategy.py` | Spot strategy (loaded by `freqtrade`) |
| `user_data/strategies/MarketStrategy.py` | Futures strategy (loaded by `freqtrade-futures`) |
