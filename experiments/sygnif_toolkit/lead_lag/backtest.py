...
def run_backtest(fills_file, events_file):
    ...
    for _, fill in df_fills.iterrows():
        ...
        prior_events = df_events[
            (df_events["ts"] <= fill_ts) & (df_events["ts"] >= fill_ts - 500)
        ]
        prior_events = prior_events[
            prior_events["data"].apply(
                lambda d: d["direction"] == direction
                and d["leader"] in ["coinbase", "kraken"]
            )
        ]
        ...
        drift_bps = (
            (
                (
                    fill.get("_ground_truth", {}).get(
                        "mid_5s_after_entry", fill["price"]
                    )
                    - fill["price"]
                )
                / fill["price"]
    ...
    follow_through_rate = (
        sum(1 for d in confirmed_fills if d > 0) / n_confirmed if n_confirmed > 0 else 0
    )
    ...
    print(
        f"Mean post-fill drift (confirmed): {mean_conf:.2f} bps, 95% CI: [{ci_conf[0]:.2f}, {ci_conf[1]:.2f}]"
    )
    print(
        f"Mean post-fill drift (unconfirmed): {mean_unconf:.2f} bps, 95% CI: [{ci_unconf[0]:.2f}, {ci_unconf[1]:.2f}]"
    )
