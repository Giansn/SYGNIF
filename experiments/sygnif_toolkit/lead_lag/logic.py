...
def compute_mid_velocity(df_book, window_ms=2000, lag_ms=500):
    ...
    if "bid" in df_book.columns and "ask" in df_book.columns:
        ...
        df_book["mid"] = df_book[
            "price"
        ]  # Fallback for simplicity if just price is provided
    ...
    df_grid["mid_velocity"] = (df_grid["mid"] - df_grid["mid_t_500"]) / df_grid[
        "mid_t_500"
    ]
...
def detect_lead_events(df_kraken, df_cb, df_bybit):
    ...
    merged["kraken_std"] = (
        merged["kraken_vel"].rolling(min_periods=100, window=window_30m).std()
    )
    merged["cb_std"] = (
        merged["cb_vel"].rolling(min_periods=100, window=window_30m).std()
    )
    merged["bybit_std"] = (
        merged["bybit_vel"].rolling(min_periods=100, window=window_30m).std()
    )
