...
def simulate_btc_price(days=60, start_price=60000, volatility=0.04, dt=1.0):
    ...
    for t in range(1, n_steps):
        ...
        # GBM formula
        prices[t] = prices[t-1] * np.exp(-0.5 * volatility**2 * dt + volatility * np.sqrt(dt) * z)
...
def run_golden_cross_strategy(df):
    ...
    for date, row in df.iterrows():
        ...
        if row['Position'] == 1.0 and portfolio['cash'] > 0:
            ...
            portfolio['cash'] = 0
            ledger.append({'Date': date, 'Action': 'BUY', 'Price': price, 'BTC': btc_to_buy, 'Portfolio Value': portfolio['btc'] * price})
        ...
        # Sell Signal
            ...
            portfolio['btc'] = 0
            ledger.append({'Date': date, 'Action': 'SELL', 'Price': price, 'BTC': btc_sold, 'Portfolio Value': portfolio['cash']})
...
def generate_fills_fixture(filename="fixtures/fills.jsonl", num_trades=200):
    ...
    for i in range(num_trades):
        ...
        slippage_bps = np.random.normal(2, 1) if not is_maker else np.random.normal(0, 0.5)
        price_diff = intended_price * (slippage_bps / 10000)

        actual_price = float(intended_price + (price_diff * side)) # Buy higher, sell lower = adverse
        ...
        fill = {
            "ts": int(intended_ts + np.random.randint(50, 200)), # Actual fill slightly after intent
            "side": side,
            ...
            "_ground_truth": {
                ...
                "funding_rate": round(np.random.normal(0.0001, 0.00005), 6) # Fake funding
            }
        }
        fills.append(fill)
