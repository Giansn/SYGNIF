    ...
    for fill in trades:
        if open_position is None:
            ...
            if open_position["side"] != fill["side"]:
                ...
                # Signal:
                signal_pnl = (
                    (exit_fill["intended_price"] - entry["intended_price"]) * qty * side
                )
                ...
                # Adverse Selection
                mid_5s_entry = entry.get("_ground_truth", {}).get(
                    "mid_5s_after_entry", entry["price"]
                )
                mid_5s_exit = exit_fill.get("_ground_truth", {}).get(
                    "mid_5s_after_entry", exit_fill["price"]
                )
                adverse_selection_entry = side * (mid_5s_entry - entry["price"]) * qty
                adverse_selection_exit = (
                    -side * (mid_5s_exit - exit_fill["price"]) * qty
                )
                ...
                entry_slippage_additive = (
                    (entry["intended_price"] - entry["price"]) * qty * side
                )
                exit_slippage_additive = (
                    (exit_fill["price"] - exit_fill["intended_price"]) * qty * side
                )
                ...
                if abs(residual) > 0.01:
                    print(
                        f"Residual check failed for trade! {residual}", file=sys.stderr
                    )
