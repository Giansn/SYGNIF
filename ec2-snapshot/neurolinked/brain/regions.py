...
class LanguageRegion(BrainRegion):
    """Dedicated language-encoding region.

    CLAUDE.md §4 spec: 'planned 3000-4000 | language | reserved for Phase 2'.
    Encodes text features (Model2Vec embeddings injected via the brain's
    text channel) with sparse representations like concept_layer, but with
    higher firing thresholds tuned for symbolic / discrete-token patterns.

    V1: same Izhikevich params as concept_layer (intrinsically bursting),
    sparser winner-take-all (top 2% rather than 5%). Specialization (semantic
    embedding STDP shaping) is V2 work.

    Opt-in: SYGNIF_BRAIN_LANGUAGE_REGION=1 in service env. Requires brain
    state reset on first activation (changes total neuron count).
    """

    def __init__(self, n_neurons: int, dt: float = 1.0):
        # Use concept_layer params if no language-specific params defined
        params = BrainConfig.NEURON_PARAMS.get(
            "language",
            BrainConfig.NEURON_PARAMS["concept_layer"])
        super().__init__("language", n_neurons, params, dt)
        self.position = np.array([0.4, 0.4, 0.4])  # offset in 3D from concept
        self.wta_strength = 3.0  # tighter inhibition than concept (2.0)

    def step(self, external_input: np.ndarray, t: float) -> np.ndarray:
        # Tighter WTA than concept_layer: only top 2%
        fired = super().step(external_input, t)
        if np.sum(fired) > 0:
            k = max(1, int(self.n_neurons * 0.02))
            potentials = self.neurons.v.copy()
            potentials[~fired] = -100
            if np.sum(fired) > k:
                threshold = np.partition(potentials, -k)[-k]
...
def create_all_regions(total_neurons: int = None, dt: float = 1.0) -> dict:
    ...
    # 2026-05-10: opt-in language region (CLAUDE.md §4 spec).
    # Set SYGNIF_BRAIN_LANGUAGE_REGION=1 to enable. Adds NEW neurons to the
    # brain (default 1000) — requires brain state reset on first activation.
    import os as _os
    if _os.environ.get("SYGNIF_BRAIN_LANGUAGE_REGION", "0") == "1":
        n_lang = int(_os.environ.get("SYGNIF_BRAIN_LANGUAGE_NEURONS", "1000"))
        regions["language"] = LanguageRegion(n_lang, dt=dt)
