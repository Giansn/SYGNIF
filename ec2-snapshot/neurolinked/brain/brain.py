...
# brain_state/ relative to this file: brain/brain.py -> ../brain_state/
...
class Brain:
    ...
    def step(self):
        ...
        # Live status flush — throttled to ~1 Hz so external readers (port 8890
        # insights service) get real-time step/sps without GIL-contending with
        # the brain's own HTTP server. Atomic write, never raises into step().
        now_t = time.time()
        if now_t - getattr(self, "_last_live_write", 0.0) >= 1.0:
            self._last_live_write = now_t
            try:
                self._write_live(now_t)
            except Exception:
                pass

    def _write_live(self, now_t: float) -> None:
        """Flush a tiny live.json with current step + sps + neuromods."""
        payload = {
            "ts": now_t,
            "step_count": int(self.step_count),
            "steps_per_second": round(float(self.steps_per_second), 2),
            "uptime": round(now_t - self.start_time, 1),
            "development_stage": self.development_stage,
            "total_neurons": int(self.total_neurons),
            "total_synapses": int(self.total_synapses),
            "neuromodulators": {k: round(float(v), 3) for k, v in self.neuromodulators.items()},
        }
        tmp = _LIVE_PATH + ".tmp"
