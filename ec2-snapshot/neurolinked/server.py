...
# 2026-05-10: DLP Phase 2.A — salience head + response composer
from brain.output_classifier import SalienceClassifier
from brain import composer as _composer
...
salience_classifier: SalienceClassifier = None  # initialized after brain
...
# Dedicated single worker for ``POST /api/chat`` — keeps NumPy brain reads + Bybit context + Haiku
# off the asyncio loop so ``GET /api/sygnif/summary`` and dashboards stay responsive.
_CHAT_EXECUTOR: concurrent.futures.ThreadPoolExecutor | None = None


def _chat_executor() -> concurrent.futures.ThreadPoolExecutor:
    global _CHAT_EXECUTOR
    if _CHAT_EXECUTOR is None:
        _CHAT_EXECUTOR = concurrent.futures.ThreadPoolExecutor(
            max_workers=1,
            thread_name_prefix="nl_api_chat",
        )
...
def init_brain():
    ...
    # DLP Phase 2.A salience head — read-only sampler
    global salience_classifier
    try:
        salience_classifier = SalienceClassifier(brain)
    except Exception as _e:
        import logging as _log
        _log.warning("salience_classifier init failed: %s", _e)
        salience_classifier = None
...
def simulation_loop():
    ...
    # Default 18 Hz keeps the GIL free enough for FastAPI + ``/api/sygnif/ping`` on the same process.
    # Raise with SYGNIF_NEUROLINKED_SIM_TARGET_HZ for bench/research; lower if HTTP still stalls.
    raw_hz = (os.environ.get("SYGNIF_NEUROLINKED_SIM_TARGET_HZ") or "18").strip() or "18"
    try:
        hz = float(raw_hz)
    except ValueError:
        hz = 100.0
    ...
    target_dt = 1.0 / hz
    _raw_yield = (os.environ.get("SYGNIF_NEUROLINKED_POST_STEP_YIELD_SEC") or "0.035").strip() or "0.035"
    try:
        post_step_yield = float(_raw_yield)
    except ValueError:
        post_step_yield = 0.035
    post_step_yield = max(0.0, min(0.2, post_step_yield))
    while sim_running:
        start = time.time()
        try:
            ...
            # When ``brain.step()`` exceeds ``target_dt``, the frame sleep at the bottom is ~0; keep a
            # floor tied to ``target_dt`` so large brains still time-slice with the asyncio thread.
            adaptive_yield = max(post_step_yield, min(0.1, 0.5 * target_dt))
            adaptive_yield = min(0.2, adaptive_yield)
            if adaptive_yield > 0:
                time.sleep(adaptive_yield)
...
async def get_state():
    if not brain:
        return JSONResponse({"error": "Brain not initialized"}, status_code=503)
    _raw_st = (os.environ.get("SYGNIF_NEUROLINKED_STATE_THREAD_TIMEOUT_SEC") or "12").strip() or "12"
    try:
        _state_thread_to = float(_raw_st)
    except ValueError:
        _state_thread_to = 12.0
    _state_thread_to = max(3.0, min(60.0, _state_thread_to))
    try:
        async with _brain_io_sem():
            state = await asyncio.wait_for(
                asyncio.to_thread(brain.get_state),
                timeout=_state_thread_to,
            )
        return JSONResponse(state)
    except asyncio.TimeoutError:
        return JSONResponse(_state_degraded_payload("get_state_timeout"))
        ...
        skip_bridge = (
            ...
            or text.startswith(
                (
                    ...
                    "SWARM_BEE",
                    "MARKET_FEED",
                    "MARKET ",  # space-prefixed market tape (BTCUSDT, ETHUSDT, ...) — skip auto_recall
                    "BYBIT_SWARM_LAYER",
...
def _local_network_hive_reply(user_text: str, trading_context: str) -> str | None:
    ...
    Only invoked when the client explicitly requests a digest (``hivemind_digest_first`` in
    ``POST /api/chat``) or when ``SYGNIF_NEUROLINKED_CHAT_DIGEST_FIRST=1`` is set and the JSON
    body omits the flag. Normal free-text chat goes straight to Haiku / ``BrainTextDecoder``.
    """
    lines: list[str] = []
...
def _chat_process_sync(text: str, executive_boost: bool, digest_first: bool) -> dict:
    """
    Full ``/api/chat`` CPU + blocking I/O path (runs in ``_chat_executor`` worker thread).

    Must not run on the uvicorn asyncio loop: ``get_brain_summary`` + ``_build_trading_context``
    (Bybit) + optional Haiku otherwise stall every other HTTP client (``sygnif chat``, summary, …).
    """
    features = text_encoder.encode(text)
    ...
    response = _local_network_hive_reply(text, context) if digest_first else None
    if not response:
        response = _haiku_reply(text, summary, context)
    if not response:
...
def _digest_first_from_chat_body(data: dict) -> bool:
    """
    Prefer explicit JSON. ``bool("false")`` is True in Python — never use bare ``bool()`` on strings.

    If ``hivemind_digest_first`` is absent, fall back to env ``SYGNIF_NEUROLINKED_CHAT_DIGEST_FIRST``.
    """
    if "hivemind_digest_first" not in data:
        return _nl_env_default("SYGNIF_NEUROLINKED_CHAT_DIGEST_FIRST", "0").lower() in (
            "1",
            "true",
            "yes",
            "on",
        )
    raw = data.get("hivemind_digest_first")
...
@app.get("/api/brain/salience")
async def brain_salience():
    """Classify the current brain state into a salience label."""
    if salience_classifier is None:
        return {"error": "salience_classifier not initialized"}
    return salience_classifier.classify()


@app.post("/api/brain/compose")
async def brain_compose(data: dict):
    """Compose a grounded English string from {label, ctx}.

    Body: {"label": "routine|alert|risk_event|trend_change|win|loss|explain",
           "ctx":   {required slots per template}}
    """
    label = data.get("label", "routine")
    ctx = data.get("ctx", {}) or {}
    try:
        out = _composer.compose(label, ctx)
        return {"status": "ok", **out}
    except _composer.MissingSlot as e:
        return {"status": "missing_slot", "error": str(e)}
    except _composer.NoTemplate as e:
        return {"status": "no_template", "error": str(e)}


@app.post("/api/brain/salience/refine")
async def brain_salience_refine(data: dict):
    """STDP-correlation refinement of the salience head.

    Body: {"outcome": "win|loss|routine|alert|risk_event|trend_change|explain",
           "lr":      0.05  (optional, default 0.05)}

    Called by sidecar services (brain_context_publisher) when trade.close
    fires — passes the actual outcome label so the classifier nudges
    weights toward it. Closes the learning loop.
    """
    if salience_classifier is None:
        return {"error": "salience_classifier not initialized"}
    ...
    if not outcome:
        return {"error": "missing 'outcome' field"}
    try:
        salience_classifier.refine_from_outcome(outcome, lr=lr)
        # Return current label distribution so caller can verify learning
        from brain.output_classifier import LABELS
        import numpy as _np
        dist = {LABELS[i]: int((salience_classifier.preferred == i).sum())
                for i in range(len(LABELS))}
        return {"status": "ok", "outcome": outcome, "lr": lr,
                "label_distribution": dist,
                "total_sampled": int(len(salience_classifier.preferred))}
    except Exception as e:
        return {"error": f"{type(e).__name__}: {e}"}


@app.get("/api/brain/narrate")
async def brain_narrate():
    """One-shot: classify brain state + compose using auto-built minimal ctx.

    The auto-ctx is intentionally minimal — for richer ctx, callers should
    POST to /api/brain/compose with their own assembled ctx including
    discovery snapshot data + portfolio state.
    """
    if salience_classifier is None:
        return {"error": "salience_classifier not initialized"}
    sal = salience_classifier.classify()
    label = sal["label"]
    # Minimal auto-ctx that satisfies the routine_NORMAL_v1 template's required
    # slots. Real callers should POST richer ctx.
    ...
    ctx = {
        ...
        "open_count":             0,
        "max_open":               7,
        "total_unrealized_usdc":  0.0,
        "equity_usdc":            0.0,
        "minutes_in_regime":      0,
        "iv_atm_pct":             0.0,
        "iv_rv":                  0.0,
        "implied_1d_move_usd":    0,
        "ts_utc":                 _time.strftime("%Y-%m-%dT%H:%M:%SZ", _time.gmtime()),
    }
    try:
        out = _composer.compose(label, ctx)
        return {"salience": sal, "composed": out}
    except _composer.MissingSlot as e:
        return {"salience": sal, "error": f"missing_slot: {e}",
                "hint": "POST /api/brain/compose with richer ctx"}
    except _composer.NoTemplate as e:
        return {"salience": sal, "error": f"no_template: {e}"}
...
async def chat(request: Request):
    ...
    digest_first = _digest_first_from_chat_body(data)
    loop = asyncio.get_running_loop()
    try:
        out = await loop.run_in_executor(
            _chat_executor(),
            partial(_chat_process_sync, text, executive_boost, digest_first),
        )
    except Exception as e:
        return JSONResponse(
            {"error": str(e), "response": "", "state": {}},
            status_code=500,
        )
    return out
...
def _read_nl_ping_payload() -> dict:
    """Tiny snapshot for load checks — no heavy NumPy; safe on the asyncio loop."""
    if not brain:
        return {"ok": False, "error": "no_brain"}
    try:
        uptime = round(time.time() - brain.start_time, 1)
    except Exception:
        uptime = 0.0
    return {
        "ok": True,
        "step": int(getattr(brain, "step_count", 0)),
        "stage": getattr(brain, "development_stage", ""),
        "uptime": uptime,
    }


def _brain_summary_degraded_payload(reason: str) -> dict:
    pred = brain.regions.get("predictive") if brain else None
    surprise = 0.0
    if pred is not None:
        try:
            surprise = float(getattr(pred, "surprise", 0.0))
        except Exception:
            pass
    pong = _read_nl_ping_payload()
    out: dict = {
        "degraded": True,
        "degraded_reason": reason,
        "step": pong.get("step", -1),
        "stage": pong.get("stage", "") or "?",
        "uptime": pong.get("uptime", 0.0),
    ...
    if claude_bridge:
        try:
            out["interaction_count"] = int(claude_bridge._interaction_count)
            ctx = claude_bridge._current_context
            out["current_context"] = ctx if isinstance(ctx, str) else str(ctx)
        except Exception:
            pass
    return out


def _state_degraded_payload(reason: str) -> dict:
    if not brain:
        return {"degraded": True, "degraded_reason": reason, "error": "no_brain"}
    try:
        uptime = round(time.time() - brain.start_time, 1)
    except Exception:
        uptime = 0.0
    return {
        "step": int(getattr(brain, "step_count", 0)),
        "total_neurons": int(getattr(brain, "total_neurons", 0)),
        "total_synapses": int(getattr(brain, "total_synapses", 0)),
        "steps_per_second": round(float(getattr(brain, "steps_per_second", 0.0)), 2),
        "development_stage": getattr(brain, "development_stage", ""),
        "neuromodulators": {},
        "regions": {},
        "region_firing": {},
        "safety": {},
...
async def _brain_summary_response():
    ...
    raw_to = (os.environ.get("SYGNIF_NEUROLINKED_SUMMARY_THREAD_TIMEOUT_SEC") or "15").strip() or "15"
    try:
        thread_to = float(raw_to)
    except ValueError:
        thread_to = 15.0
    thread_to = max(3.0, min(60.0, thread_to))

    def _run_summary():
        return claude_bridge.get_brain_summary()

    try:
        # Off the event loop (same pattern as ``GET /api/state``) so ingest + sim
        # cannot starve socket polling. Bounded wait → degraded JSON instead of HTTP timeout.
        async with _brain_io_sem():
            result = await asyncio.wait_for(
                asyncio.to_thread(_run_summary),
                timeout=thread_to,
            )
        return JSONResponse(json.loads(json.dumps(result, default=str)))
    except asyncio.TimeoutError:
        degraded = _brain_summary_degraded_payload("summary_thread_timeout")
        return JSONResponse(json.loads(json.dumps(degraded, default=str)))
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=500)


@app.get("/api/sygnif/ping")
async def sygnif_nl_ping():
    """Ultra-light HTTP reachability + step counter (no ``to_thread``)."""
    payload = _read_nl_ping_payload()
