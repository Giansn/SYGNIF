---
name: finance-agent
description: "Sygnif unified domain: strategy code + live markets + Telegram bot (finance_agent only). Market/TA/plays/overseer, SygnifStrategy/MarketStrategy2, GitNexus. Triggers: crypto, TA, signals, trades, /ta, /btc, /finance-agent, NFI, entry tags, sf_*, orb_*, NewHedge, MTF RSI, parity, bot.py."
allowed-tools:
  - Agent
  - Task
  - Bash
...
**Canonical skill path (version control):** Sygnif repo — **`.cursor/skills/finance-agent/SKILL.md`** (clone path usually **`~/SYGNIF`**). Cursor loads workspace skills from **`.cursor/skills/`**. **Telegram** commands are implemented only in **`finance_agent/bot.py`** (this skill documents parity); BTC-only **analysis tools** (offline JSON pull, snapshot layout) live under **`finance_agent/btc_specialist/`** and the **btc-specialist** Cursor skill — not a second bot.
...
**v6.2 (Cursor-canonical skills):** Repo skills live under **`.cursor/skills/`** only (no **`.claude/skills`** mirror for finance-agent/btc-specialist). **btc-specialist** skill narrows to BTC analysis tools; **finance-agent** retains sole Telegram bot documentation.
