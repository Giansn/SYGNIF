...
2. **`prediction_agent/btc_predict_runner.py`** — the **Sygnif-local** path: trains/evaluates **RandomForest**, **XGBoost**, and **LogisticRegression** on OHLCV from **`finance_agent/btc_specialist/data/`** (`btc_1h_ohlcv.json`, `btc_daily_90d.json`). Those files are fed by **`research/nautilus_lab/bybit_nautilus_spot_btc_training_feed.py`** (Docker **`nautilus-research`** in **`docker-compose.btc-nautilus-research.yml`**) using the **Nautilus Bybit HTTP adapter** — spot **BTC/USDT** only; see also **`nautilus_spot_btc_market_bundle.json`** for ticker/trades/order book snapshot metadata. No paid APIs for the runner itself; document CLI flags (`--timeframe`, `--window`).
