...
## [`HANDOFF.md`](HANDOFF.md) — picking this up cold

State, what is and is not verified, open work in priority order, decisions
already made, and the traps. Start there if you have no context — including the
warning that the .NET SDK was installed by hand into an ephemeral container and
must be reinstalled before `verify.sh` will fully pass.
