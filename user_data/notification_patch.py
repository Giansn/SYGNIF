...
content = content.replace(exit_re.group(0), r'''    def _format_exit_msg(self, msg: RPCExitMsg) -> str:  # NOTIFICATION_PATCH_V3
        ...
        if is_fill:
            ...
            # Exit reason mapping
            er = exit_reason
            if "stoploss_on_exchange" in er: desc = "Exchange stoploss (doom)"
            elif "trailing_stop_loss" in er: desc = "Trailing exit (ratchet)"
            ...
            elif "force_exit" in er: desc = "Manual force exit"
            elif "emergency" in er: desc = "Emergency exit"
            elif "liquidation" in er: desc = "Liquidation"
            elif "roi" in er: desc = "ROI target"
